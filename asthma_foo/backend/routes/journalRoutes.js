...
const JournalEntry = require("../models/JournalEntry"); // ✅ Ensure this is the correct model

const router = express.Router();

// POST request to save a new journal entry
router.post("/", async (req, res) => {
  console.log("🚀 Received request at POST /api/journal"); // ✅ Debugging log

  try {
    const { userId, asthmaAttack, symptoms, comments, dateCreated } = req.body;
    console.log("📥 Received journal entry:", req.body); // Debugging
    ...
    const newEntry = new JournalEntry({
      ...
      symptoms,
      comments,
      dateCreated: date ? new Date(date) : new Date(), // ✅ Store the date in ISO format
    });
...
router.get("/:userId", async (req, res) => {
  try {
    console.log("🚀 Received request at GET /api/journal"); // ✅ Debugging log
